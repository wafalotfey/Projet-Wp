(function ($) {
    ...
    wp.aromaticcustomizerRepeater = {
        ...
        init: function () {
            $('.iconpicker-items>i').on('click', function () {
                ...
                let result = iconClass.indexOf("ja");
				if(result==1){
					  var iconElement = '<i class="jcs '.concat(iconClass, '"><\/i>');
				}else{	
                ...
                aromatic_customizer_repeater_refresh_social_icons(th);
                return false;
            });
        },
    ...
    $(document).ready(function () {
        wp.aromaticcustomizerRepeater.init();

        $('.iconpicker-search').on('keyup', function () {
            wp.aromaticcustomizerRepeater.search($(this));
        });

        $('.icp-auto').on('click', function () {
            wp.aromaticcustomizerRepeater.iconPickerToggle($(this));
        });
