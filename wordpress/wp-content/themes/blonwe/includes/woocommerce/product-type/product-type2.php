...
if(!function_exists('blonwe_product_type2')){ 
	function blonwe_product_type2($stockprogressbar = '', $stockstatus = '', $shippingclass = '', $countdown = '', $product_sku = '', $productattributes = ''){
		global $product;
		global $post;
		global $woocommerce;
		...
		$postview  = isset( $_POST['shop_view'] ) ? $_POST['shop_view'] : '';
			...
			ob_start();
			do_action('blonwe_product_box_footer', $stockprogressbar, $stockstatus, $shippingclass, $countdown = 'false', $product_sku, $productattributes);
			$output .= ob_get_clean();
